...
var baiduMp3Path=path.resolve(__dirname,"../","../","public","uploads","baidu-mp3");
...
exports.startSocket = function(server) {
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('send-text', function(data) {
        	var text=data.data;
            console.log(">>>>>>>>>>>"+text);
            baiduClient.text2audio(text).then(function(result) {
                if (result.data) {
                    fs.writeFileSync(path.join(baiduMp3Path,"baidutest.mp3"), result.data);
                    socket.emit('play-video', { data: '/uploads/baidu-mp3/baidutest.mp3' })
                } else {
                    // 服务发生错误
                    console.log(result)
                }
